...
public class OrdersServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ("confirmAll".equals(action)) {
                ...
                if (updatedRows > 0) {
                    response.setStatus(HttpServletResponse.SC_OK);
                    response.getWriter().write("{\"success\":true,\"message\":\"Đã xác nhận thành công và cập nhật kho " + updatedRows + " đơn hàng!\"}");
                } else {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    response.getWriter().write("{\"success\":false,\"message\":\"Không tìm thấy đơn hàng nào đang ở trạng thái chờ xử lý!\"}");
                }
                return;
            }
            ...
            switch (action) {
                case "confirm":
                    ...
                    if (orderConfirm != null) {
                        notificationDAO.insertNotification(orderConfirm.getUserId(), "🛒 Đơn hàng đã được xác nhận", "Đơn hàng #" + orderConfirm.getOrderCode() + " của bạn đã được SportStore xác nhận thành công.", "/account");
                    }
                    break;
                ...
                case "cancel":
                    ...
                    if (orderCancel != null) {
                        notificationDAO.insertNotification(orderCancel.getUserId(), " Đơn hàng đã bị hủy", "Đơn hàng #" + orderCancel.getOrderCode() + " đã bị hủy bởi hệ thống. Vui lòng liên hệ hỗ trợ nếu có thắc mắc.", "/account");
                    }
                    break;

                case "shipping":
                    Order order = orderDAO.getOrderById(id);
                    if (order == null) throw new RuntimeException("Không tìm thấy thông tin đơn hàng này!");
                    ...
                    notificationDAO.insertNotification(order.getUserId(), " Đơn hàng đang được giao", "Đơn hàng #" + order.getOrderCode() + " đã bàn giao cho đối tác GHN. Mã vận đơn: " + ghnCode, "/account");
                    break;
                ...
                case "complete":
                    ...
                    if (orderComplete != null) {
                        notificationDAO.insertNotification(orderComplete.getUserId(), " Đơn hàng hoàn thành", "Đơn hàng #" + orderComplete.getOrderCode() + " đã giao thành công. Cảm ơn bạn đã mua sắm tại SportStore!", "/account");
                    }
                    break;
                ...
                case "approve_refund":
                    ...
                    if (refundOrder == null) throw new RuntimeException("Không tìm thấy đơn hàng cần phê duyệt hoàn tiền!");
                    orderDAO.approveRefund(id);

                    notificationDAO.insertNotification(refundOrder.getUserId(), " Phê duyệt hoàn tiền", "Yêu cầu hoàn tiền cho đơn hàng #" + refundOrder.getOrderCode() + " đã được phê duyệt thành công.", "/account");
                    break;

                case "rejectRefund":
                case "reject_refund":
                    Order rejectOrder = orderDAO.getOrderById(id);
                    if (rejectOrder == null) throw new RuntimeException("Không tìm thấy đơn hàng cần từ chối hoàn tiền!");
                    orderDAO.rejectRefund(id);

                    notificationDAO.insertNotification(rejectOrder.getUserId(), "⚠ Từ chối hoàn tiền", "Yêu cầu hoàn tiền cho đơn hàng #" + rejectOrder.getOrderCode() + " đã bị từ chối.", "/account");
                    break;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            ...
            try {
                ...
                if (shippingOrders != null && !shippingOrders.isEmpty()) {
                    ...
                    for (Order order : shippingOrders) {
                        ...
                        if (ghnCode == null || ghnCode.isEmpty()) continue;
                        ...
                        if (result.has("code") && result.get("code").getAsInt() == 200) {
                            ...
                            } else if (ghnStatus.contains("return") || ghnStatus.contains("refund") || "damage".equals(ghnStatus) || "lost".equals(ghnStatus)) {
                                java.sql.Connection conn = null;
                                try {
                                    conn = com.shop.sportstore.untils.DBConnection.getConnection();
                                    conn.setAutoCommit(false);
                                    ...
                                    orderDAO.increaseProductStock(order.getId(), conn);
                                    conn.commit();
                                    notificationDAO.insertNotification(order.getUserId(), "⚠ Đơn hàng gặp sự cố", "Đơn hàng #" + order.getOrderCode() + " đang được hoàn trả lại do lỗi vận chuyển hoặc sự cố.", "/account");
                                } catch (Exception e) {
                                    if (conn != null) conn.rollback();
                                    throw e;
                                } finally {
        ...
        if (status == null || status.trim().isEmpty()) {
            ...
            String checkStatus = status.trim().toUpperCase();
            switch (checkStatus) {
                case "PENDING":
                case "CHO_XAC_NHAN":
                    checkStatus = "PENDING";
                ...
                case "REFUND_PENDING":
                case "PENDING_REFUND":
                    checkStatus = "PENDING_REFUND";
                    break;
                case "REFUNDED":
                    checkStatus = "REFUNDED";
                    break;
                case "REFUND_REJECTED":
                    checkStatus = "REFUND_REJECTED";
                    break;
                default:
                    checkStatus = "PENDING";
            }
            orders = orderDAO.getOrdersByStatus(checkStatus);
        }
